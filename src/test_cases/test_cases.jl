...
let
    frequencies = (1.0:0.01:300.0)u"GHz"  # Frequencies in GHz
    atm = upscale_atm_res(standard_atm, Int(200))
    function calculate_upwelling_intensity(myfreq)  # Frequency
        λ = Unitful.c / myfreq  # Convert wavelength to frequency in GHz
        I0_down = B(λ, 0.00001u"K")  # Initial downwelling intensity
        I0_up = zero(typeof(I0_down))  # Initial upwelling intensity
        view_zenith_angle = 0u"°"  # Zenith angle, degrees
        ...
        # Solve the RTE and calculate upwelling intensity
        solution = solve_rte_only_TOA_SFC(model).up
        return solution  # Return the result
    end

    upwelling_intensities = calculate_upwelling_intensity.(frequencies)

    # Plot the upwelling intensity vs frequency
    p = plot(
        frequencies,
        collect(upwelling_intensities),
        xlabel="Frequency (GHz)",
        ylabel="Upwelling Intensity (W/m^2/μm)",
        title="Upwelling Intensity vs Frequency",
        label="Upwelling Intensity",
        dpi = 500
    )
    savefig(joinpath(testvisdir, "upwelling_intensity_vs_frequency.png"))
end